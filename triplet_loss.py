...
def compute_squared_distance(feat1, feat2):
    return torch.pow(feat1, 2).sum(axis=-1) + torch.pow(feat2, 2).sum(axis=-1) - 2 *  torch.mul(feat1, feat2).sum(axis=-1)

class OrderTripletLoss(nn.Module):
    def __init__(self, device):
        super().__init__()
        self.device = device
        # self.margin_dict = {0: 0.2, 1: 0.4, 2: 0.6, 3: 0.8, 4: 1.0}

    def forward(self, feats, labels, cls1, cls2, **kwargs):
        # cls1 <= cls2 when label is 0
        clsdiff = torch.abs(cls1 - cls2)
        margins = clsdiff / 5 + 0.2
        mask  = 2 * labels - 1
        # squared distances
        dref1 = compute_squared_distance(feats[0], feats[1])
        dref2 = compute_squared_distance(feats[0], feats[2])
        # loss  = torch.mul(mask, dref1 - dref2)
        # loss  = torch.max(loss, torch.tensor([0.]).to(self.device))
        diff  = torch.mul(mask, dref1 - dref2)
        loss  = torch.max(margins-diff, torch.tensor([0.]).to(self.device))
        return loss.mean()